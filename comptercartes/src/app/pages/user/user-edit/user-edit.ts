...
export class UserEditPage implements OnInit {
  ...
  isValid(): boolean {
    this.error = [];
    if (!this.isValidString(this.user.name, 3, 25)) {
      this.error.push(('Invalid name: 3 to 25 chars'));
    }
  ...
  deleteAccount() {
    this.alertCtrl.create({
      message: 'Do you really want to delete your account ' + this.user.name  +  '?<br>All data will be removed !!',
      buttons: [
